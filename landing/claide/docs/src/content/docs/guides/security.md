---
title: Security
description: Security features and best practices for Claide
---

Claide is designed with security as a core concern. This guide covers the built-in security features and best practices for production deployments.
...
claide gateway --containerized apple
```
...
Claide includes a multi-layered prompt injection detector that runs on all LLM inputs:
...
```bash
export CLAIDE_SAFETY_ENABLED=true  # default
```
...
```bash
export CLAIDE_SAFETY_LEAK_DETECTION_ENABLED=true  # default
```
...
- `CLAIDE_MASTER_KEY` environment variable (hex-encoded 32 bytes)
- A key file
- Interactive prompt
...
1. **Always use container isolation in production** — Run `claide gateway --containerized`
...
9. **Encrypt secrets at rest** — Use `claide secrets encrypt` to protect API keys in config
