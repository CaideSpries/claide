---
title: Plugins
description: Extend Claide with custom tools using JSON manifest plugins
---

The plugin system lets you add custom tools to your agent without modifying Claide's source code. Plugins are JSON manifest files that define a tool's name, parameters, and a command template.

## Plugin structure

Create a JSON file in `~/.claide/plugins/`:
...
1. **Discovery** — At startup, Claide scans configured plugin directories for JSON files
...
```json
{
  "plugins": {
    "enabled": true,
    "directories": ["~/.claide/plugins", "/opt/claide/plugins"]
  }
}
```
