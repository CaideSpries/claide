...
description: Step-by-step guide to migrate your OpenClaw setup to Claide
---

If you're running OpenClaw today, Claide gives you a lighter, faster, more secure runtime with most of the same integrations. This guide walks through what maps directly, what needs adaptation, and how to move your setup over.

## What you get

Claide is a ground-up rewrite in Rust that keeps the best parts of OpenClaw's integration model while dropping the Node.js runtime:
...
1. **Install Claide** — see the [installation guide](/docs/getting-started/installation/)
2. **Try the automated migration** — `claide migrate` auto-detects your OpenClaw installation and converts config + skills:
   ```bash
   ...
   claide migrate --from ~/.openclaw --yes
   ```
   The command backs up your existing Claide config before writing changes.
...
> **Tip:** `claide migrate` handles the config conversion automatically. The mapping below is for reference or manual migration.

OpenClaw uses `~/.openclaw/openclaw.json` (JSON5). Claide uses `~/.claide/config.json` (strict JSON). The structure is flatter and uses `snake_case` throughout.

### Field mapping

| OpenClaw | Claide | Notes |
|----------|-----------|-------|
...
| `session.scope` | — | Claide uses container-per-request isolation instead |
...
This is the easiest part. Claide's skill loader is directly compatible with OpenClaw's skill format.

### Steps

The `claide migrate` command copies skills automatically. To do it manually:

1. Copy your skills directory:
   ```bash
   cp -r ~/.openclaw/skills/* ~/.claide/skills/
   ```
...
Claide reads skills with the same YAML frontmatter and markdown body format. The loader checks metadata namespaces in this priority order: `claide` → `clawdbot` → `openclaw` → raw (unnamespaced).
...
These OpenClaw-specific fields are parsed but have no effect in Claide:
...
If a skill doesn't appear in `claide skills list`, check that its YAML frontmatter is valid and that any `os` or `requires.bins` conditions are satisfied on your system.

## Plugin and extension migration

This requires more work. OpenClaw uses npm/TypeScript extensions; Claide uses JSON manifest plugins.
...
Claide plugins are JSON files in `~/.claide/plugins/<name>/plugin.json` with two execution modes:
...
**Claide plugin** (`~/.claide/plugins/github-pr/plugin.json`):
```json
{
  "name": "github_pr",
  "description": "Create a GitHub pull request",
  ...
  "command": "gh pr create --title {{title}} --head {{branch}} --body 'Created by Claide'"
}
```
...
OpenClaw ships extensions for Signal, iMessage, Matrix, Mattermost, MS Teams, Feishu, and others. For channels not natively supported by Claide, use the **webhook channel** with an external adapter:

1. Enable the webhook channel in your Claide config
2. Run a lightweight bridge that converts the platform's API to HTTP POST requests
3. Point the bridge at Claide's webhook endpoint
...
- **Long-term memory** — persistent key-value store at `~/.claide/memory/longterm.json` with categories, tags, and access tracking

### Migration steps

1. **Export key memories** — If you relied on QMD for important context, export the most valuable entries to a `MEMORY.md` file in your workspace. Claide's workspace memory tool will index it automatically.
...
3. **Session history** — Claide maintains its own conversation history in `~/.claide/sessions/`. Previous OpenClaw sessions won't carry over, but you can reference exported session files from your workspace.
...
For tools without a built-in equivalent, Claide's [MCP client](/docs/concepts/tools/) can connect to external tool servers. This is the recommended path for image understanding, audio processing, and browser automation.

OpenClaw's `tools.profile` system (per-tool execution policies) maps roughly to Claide's **approval gate**:
...
Some OpenClaw features don't have Claide equivalents:
...
Migrating isn't just about parity — Claide adds capabilities that OpenClaw doesn't have:
...
- Run `claide config check` for validation errors
...
- Run `claide agent -m "list your tools"` to see what's available
...
- OpenClaw uses JSON5 (trailing commas, comments) — Claide requires strict JSON
