...
Claide ships with 4 built-in templates:
...
```bash
# Use a template
claide agent --template researcher -m "Research Rust async patterns"

# List available templates
claide template list

# Show template details
claide template show coder
```

### Batch mode

```bash
claide batch --input prompts.txt --template coder
```
...
Create a JSON file in `~/.claide/templates/`:
...
```bash
claide agent --template devops -m "Set up a GitHub Actions pipeline"
```
