...
Claide ships with 29 built-in tools. Each tool is available to the agent by default unless restricted by the approval gate or a template's tool whitelist.
...
Stored at `~/.claide/memory/longterm.json`. Persists across sessions with access tracking.
