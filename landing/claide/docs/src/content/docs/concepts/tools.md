---
title: Tools
description: How agent tools work in Claide
---
...
Claide ships with 29 built-in tools:
...
When the LLM returns multiple tool calls in one response, Claide executes them concurrently using `futures::future::join_all`. This reduces latency when tools are independent.
...
You can add custom tools without modifying Claide's source code using the [plugin system](/docs/guides/plugins/). Plugins are JSON manifests that define tool name, parameters, and a command template.
