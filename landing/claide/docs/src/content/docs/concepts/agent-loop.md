---
title: Agent Loop
description: How Claide processes messages and executes tools
---

The agent loop is the core of Claide. It receives a message, builds context, calls an LLM, executes tool calls, and returns the response.
...
```bash
claide agent --stream -m "Explain monads"
```
...
Configure via:
```bash
export CLAIDE_AGENTS_DEFAULTS_AGENT_TIMEOUT_SECS=600
```
