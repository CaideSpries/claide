...
Providers are the LLM backends that power your agent. Claide supports 9 providers out of the box, all configurable via `~/.claide/config.json` or environment variables.
...
The onboard wizard supports Anthropic, OpenAI, and OpenRouter. For other providers, edit `~/.claide/config.json` directly or use environment variables.
...
```bash
export CLAIDE_PROVIDERS_ANTHROPIC_API_KEY=sk-ant-...
```
...
```bash
export CLAIDE_PROVIDERS_OPENAI_API_KEY=sk-...
```
...
```bash
export CLAIDE_PROVIDERS_OPENROUTER_API_KEY=sk-or-...
```
...
```bash
export CLAIDE_PROVIDERS_OLLAMA_API_KEY=ollama
```
...
```bash
export CLAIDE_PROVIDERS_GEMINI_API_KEY=AIza...
```
...
```bash
export CLAIDE_PROVIDERS_NVIDIA_API_KEY=nvapi-...
```
...
```bash
export CLAIDE_PROVIDERS_OPENAI_API_BASE=https://my-proxy.example.com/v1
```

## Provider stack

Claide wraps providers in a composable stack:
...
```bash
claide agent --stream -m "Tell me a story"
```
...
CLAIDE_PROVIDERS_OPENROUTER_API_KEY=...
CLAIDE_PROVIDERS_GROQ_API_KEY=...
CLAIDE_PROVIDERS_OLLAMA_API_KEY=...
CLAIDE_PROVIDERS_VLLM_API_KEY=...
CLAIDE_PROVIDERS_GEMINI_API_KEY=...
CLAIDE_PROVIDERS_NVIDIA_API_KEY=...
CLAIDE_PROVIDERS_ZHIPU_API_KEY=...

# Custom base URLs
CLAIDE_PROVIDERS_OPENAI_API_BASE=...
CLAIDE_PROVIDERS_OLLAMA_API_BASE=...

# Retry
CLAIDE_PROVIDERS_RETRY_ENABLED=true
CLAIDE_PROVIDERS_RETRY_MAX_RETRIES=3
CLAIDE_PROVIDERS_RETRY_BASE_DELAY_MS=1000
CLAIDE_PROVIDERS_RETRY_MAX_DELAY_MS=30000

# Fallback
CLAIDE_PROVIDERS_FALLBACK_ENABLED=true
CLAIDE_PROVIDERS_FALLBACK_PROVIDER=openai
```
